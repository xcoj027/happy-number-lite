...
export class Grade5QuizGenerator extends BaseQuizGenerator {
  ...
  private addition(): GeneratedQuiz {
    const a = this.randInt(0, 1000);
    const b = this.randInt(0, 1000);
    const correct = a + b;
    return this.makeQuiz(
      `${a} + ${b} = ?`,
      `${correct}`,
      this.wrongNumbers(correct, 100),
      'Addition',
    );
  }
  ...
  private division(): GeneratedQuiz {
    ...
    return this.makeQuiz(
      `${dividend} ÷ ${divisor} = ?`,
      `${quotient}`,
      this.wrongNumbers(quotient, 10, 1),
      'Division',
    );
  }
